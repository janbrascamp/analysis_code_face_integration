...
data_path_input = os.path.join(base_of_BIDS_tree,'derivatives','1st_level_glms','task-main_space-T1w_desc-facesVsBaseline_zStats')	#where the first-level model results are
first_level_contrast_name = 'task-main_space-MNI152NLin2009cAsym_desc-facesVsBaseline_zStats.nii.gz'
full_path_mni_template=os.path.join(base_of_BIDS_tree,'sourcedata','fsl','data','mni152_template_1mm.nii.gz')

#------------
readme_text = "This derivatives folder contains across-observer whole-brain results for the contrast between face image presentations and an implicit baseline \
that was computed at the within-observer level by "+os.path.split(data_path_input)[-1]+".py" #a description of what the derivatives folder contains
...
list_of_z_maps=[candidate_filename for candidate_filename in layout.get(scope='derivatives', return_type='file', task=first_level_contrast_entities['task'], desc=first_level_contrast_entities['desc'], suffix=first_level_contrast_entities['suffix'], extension=['nii', 'nii.gz']) if re.search("_space-"+first_level_contrast_entities['space'], candidate_filename)]
...
desc_value=re.search("desc-([a-zA-Z0-9]*)",data_path_input)[1]
