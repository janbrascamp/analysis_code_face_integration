"""
This code computes 1st level contrasts based on a covariate regressor that encodes the Euclidean distance (in 
subjective similarity space) between the current face and the preceding one, basing stats on the 5 repetitions within each observer.
...
my_high_pass=1./128.	#scans last about 280 s, and faces are presented during the central 240 s of that. But the relevant variance is random ups and downs in distance among individual face images, which are presented every 1.5 s. So can have a cut-off that removes much of the average face-
						#related activity, and still retains those ups and downs

#------------
readme_text = "This derivatives folder contains nii.gz files with whole-brain patterns of z-stat patterns a covariate regressor that encodes the Euclidean distance (in \
subjective similarity space) between the current face and the preceding one." #a description of what the derivatives folder contains.
...
#now add covariate events to models_events_of_interest:

for models_events_one_obs in models_events_of_interest:
	for models_events_one_scan in models_events_one_obs:
		
		models_events_one_scan['modulation'] = 1	#for event regressors the event value is implicity 1 if the 'modulation' column is not provided. For the covariate regressors we need to set specific values so here we create the column
		
		#get all lag-one distances:
		lag_one_distances=[]
		for face_index in range(1,models_events_one_scan.shape[0]):
			lag_one_distance_HC=np.sqrt( sum( [pow(models_events_one_scan.loc[face_index,'face_'+axis+'_loc_in_similarity_space_HC']-models_events_one_scan.loc[face_index-1,'face_'+axis+'_loc_in_similarity_space_HC'],2) for axis in ['x','y','z']] ) )
			lag_one_distance_SZ=np.sqrt( sum( [pow(models_events_one_scan.loc[face_index,'face_'+axis+'_loc_in_similarity_space_SZ']-models_events_one_scan.loc[face_index-1,'face_'+axis+'_loc_in_similarity_space_SZ'],2) for axis in ['x','y','z']] ) )
			lag_one_distance_avg=np.average([lag_one_distance_HC,lag_one_distance_SZ])
			lag_one_distances+=[lag_one_distance_avg]
		
		#normalize all lag-one distances:
		average_lag_one_distance=np.average(lag_one_distances)
		std_lag_one_distance=np.std(lag_one_distances)
		lag_one_distances_normalized=[0]+[(element-average_lag_one_distance)/std_lag_one_distance for element in lag_one_distances]
		
		#now add covariates based on normalized lag-one distances to the dataframe:
		for face_index in range(models_events_one_scan.shape[0]):		
			
			#add row to dataframe to represent the covariate, that is initially simply a copy of the row that represents the face onset we're talking about:
			models_events_one_scan.loc[models_events_one_scan.shape[0]]=[models_events_one_scan.loc[face_index,column_name] for column_name in list(models_events_one_scan.columns.values)]	
			
			#now change those two columns of the newly added row that need to be different for the covariate relative to the event itself:
			models_events_one_scan.trial_type[models_events_one_scan.shape[0]-1]='lag_one_distance'				
			models_events_one_scan.modulation[models_events_one_scan.shape[0]-1]=lag_one_distances_normalized[face_index]
...
for model_index, (model, imgs, events, confounds) in enumerate(model_and_args):	#loop over models (so over participants)
	...
	for regressor_name in ['face_image','lag_one_distance']:
		...
		for design_matrix_one_run in model.design_matrices_:
			...
			regressor_index_face_images=np.where(design_matrix_column_names.str.contains(regressor_name) & ~(design_matrix_column_names.str.contains('derivative')))[0]
		...
		entities = {
			'sub': this_sub,
			'task': task_label,
			'desc': desc_value+''.join(word.title() for word in regressor_name.split('_')),
			'run': '0',
			'suffix': 'contrastMatrix',
			'extension': '.png'
		}
